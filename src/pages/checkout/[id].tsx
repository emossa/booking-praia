...
export default function Checkout() {
  ...
  useEffect(() => {
    ...
    // console.log(nights, pricePerNight);
  }, [booking]);


  const formCheckout = useForm<RouterInputs["booking"]["createKb"]>();
  ...
  const { mutate, isLoading } = api.booking.createKb.useMutation({
    onSuccess: (data) => {
      console.log("data response", data);
      if (data) {
        toast({
          title: "Prenotazione completata",
          description: "La tua prenotazione è stata completata con successo",
          status: "success",
          duration: 9000,
          isClosable: true,
        });

        void router.push(`/thankYou/${data}`);
      }
    },
    onError: (error) => {
      console.log(error);
  ...
  return (
    <>
      ...
      <Flex
        ...
        gap={2}
      >
        <Text variant={"header"} fontSize={"3xl"}>
          Inserisci le tue informazioni e dettagli di pagamento
        </Text>
        <Flex
          mt={10}
          flexDirection={{
        ...
        >
          <Flex flexDirection={"column"} gap={5} flex={1}>
            ...
            >
              ...
              <Text>
                Di solito {booking?.roomType?.Property?.name} è molto richiesto,
                quindi ti consigliamo di prenotare il prima possibile.
              </Text>
            </Box>
            <Box>
              ...
              <Flex justifyContent={"space-between"} mt={2}>
                <Box>
                  <Text fontWeight={"bold"}>Ospiti</Text>
                  <Text>
                    {booking?.chi_adulti} Adulti, {booking?.chi_bambini}{" "}
                    Bambini, {booking?.chi_neonati} Neonati, {booking?.chi_pets}{" "}
                    Animali
                  </Text>
                </Box>
                <Flex justifyContent={"center"} alignItems={"center"}>
            ...
            >
              <Box p={2}>
                  ...
                  <Flex direction={"column"} justifyContent={"space-between"}>
                    <Box>
                      <Text fontSize={"sm"} color={"gray.400"}>
                        Tipologia alloggio:{" "}
                        {booking?.roomType?.name?.toLowerCase()}
                      </Text>
                ...
                <Flex justifyContent={"space-between"} mt={5}>
                  ...
                  <Text fontWeight={"bold"}>{booking?.price}€</Text>
                </Flex>
                {/* <Flex justifyContent={'space-between'} mt={2}>
                                    <Text>
                                        Pulizia
